...
class RingBuffer {
    public:
        ...
        uint16_t write_from(const T* const source, uint16_t size) {
            if (size > free())
                size = free();
        ...
        uint16_t read_to(T* destination, uint16_t size) {
            if (size > used())
                size = used();
    ...
    private:
        inline uint16_t wrap(const uint16_t idx) { return idx % BufferSize; }
